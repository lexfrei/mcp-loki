...
func NewQueryHandler(client *loki.Client) mcp.ToolHandlerFor[QueryParams, QueryResult] {
	return func(
		...
		resp, err := executeQuery(ctx, client, params, queryType, limit, direction)
		if err != nil {
		...
		result := QueryResult{
			...
			Count:      parsed.Count,
			Truncated:  parsed.ResultType == "streams" && parsed.Count >= limit,
...
func executeQuery(
	...
	case queryTypeInstant:
		...
		if err != nil {
			return nil, validationErr(errors.Wrap(err, "invalid instant time"))
		}

		resp, err := client.QueryInstant(ctx, params.Query, evalTime, limit)
		if err != nil {
			return nil, errors.Wrap(err, "instant query failed")
		}
	...
	default:
		...
		if err != nil {
			return nil, errors.Wrap(err, "range query failed")
		}
