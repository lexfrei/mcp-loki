...
func decodeTimestamp(raw json.RawMessage) string {
	...
	err = json.Unmarshal(raw, &asNumber)
	if err == nil {
...
func formatTimestamp(raw string, nanoseconds bool) string {
	...
	nanosecondsPart := int64(math.Round(fraction * float64(time.Second)))

	return time.Unix(int64(wholeSeconds), nanosecondsPart).UTC().Format(time.RFC3339Nano)
}
